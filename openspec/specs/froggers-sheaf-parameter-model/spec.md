...
- NOTE (pending `frogg3rs-density-documents-and-spec` delivery): slots 10 and 13 are updated ahead of that change's
...
- NOTE (pending `frogg3rs-density-documents-and-spec` delivery): Link and the old Waveshaper offset/Bias no longer exist; this scenario is updated ahead of that change's own archival so this gate does not dangle on the deleted tests in the interim. `openspec/changes/frogg3rs-density-documents-and-spec/tasks.md`'s spec-delta group restates this requirement's full scenario set at delivery.
