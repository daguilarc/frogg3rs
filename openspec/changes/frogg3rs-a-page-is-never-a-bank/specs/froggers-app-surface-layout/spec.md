...
Six requirements restate their "which of the six pages is showing"
language from "bank" to "page," matching the requirement two sections below
them that already calls the identical thing a page ("The Filter page groups
its controls by stage") — that requirement's own body still said "the
bank's Crispy," the same seam this whole change corrects, and it is restated
here too. Every occurrence of "bank" naming *parameter-model addressing*
(a parameter's own bank-and-slot address, independent of which page is on
screen) is left exactly as it reads today; this delta touches only the
display-selection sense. No requirement's SCOPE, behavior, or test changes —
this is a wording-only delta.
...
<!-- RESTATES-EXCEPT
in the per-page/bank header
  keeps: in the per-page header
-->
...
<!-- RESTATES-EXCEPT
forward arrow repeatedly from the first bank
  keeps: forward arrow repeatedly from the first page
the active bank advances one index per click
  keeps: the active page advances one index per click
a click on the last bank wraps the selection to the first
  keeps: wraps the selection to the first
the back arrow on the first bank
  keeps: the back arrow on the first page
wraps to the last bank, with exactly one bank highlighted
  keeps: wraps to the last page, with exactly one page highlighted
-->

This requirement's own title is left as "Bank selector with direct
selection" -- an openspec document heading, never shown to a player and
never read by code -- while every clause above uses "page" for the
selection concept this whole change is about; the title itself is
deferred alongside `FroggersBankId`'s rename (see `proposal.md`'s scope
decision), and not folded in here silently.
...
<!-- RESTATES-EXCEPT
the active bank uses fewer than fourteen parameter slots
  keeps: the active page uses fewer than fourteen parameter slots
the bank's parameter grid is restored in the same region
  keeps: the page's parameter grid is restored in the same region
-->
...
<!-- RESTATES-EXCEPT
chrome, bank selector, and encoder grid
  keeps: chrome, page selector, and encoder grid
-->
...
<!-- RESTATES-EXCEPT
any cell of any bank is rendered
  keeps: any cell of any page is rendered
-->
