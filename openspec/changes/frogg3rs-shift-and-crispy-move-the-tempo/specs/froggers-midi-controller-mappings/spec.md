...
The Twister preset requirement is modified as promoted. Its body names a
second shifted turn and what it does, and says that Crispy's shifted turn
moves the tempo by the same amount on every page; its "Only the Twister
carries Shift or shifted jobs" and "The Controllers page shows Crunchy's
shifted job" scenarios name both turns; three scenarios are added. Every
other clause and scenario is carried forward word for word.

One requirement is added, for the catalog line that tells the library which
action is the tempo.
...
**AND** the Twister's only encoder turn with a shifted job is the one at Crunchy's slot, and that job is Scene blend
  keeps: whose job is Scene blend
**AND** the other fifteen turns still read as one block
  keeps: still read as one block
-->
