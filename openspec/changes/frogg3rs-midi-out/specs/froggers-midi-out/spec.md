...
K = 1 is the only count at or under 100 ms at both block sizes. The note-on and the note-off before it are stamped at the frame where the detector reported, so the latency on the output timeline is the figure above, with no further delay from the block. The worst note latency SHALL be at or under 100 ms at host blocks of 128 and 256 frames. These figures were measured without the note-off on a quiet output described below. The shipped rule's latency is measured in `measure-q/report-shipped-rule.md` Table 2: 75.417 ms at 48 kHz/128-frame and 94.271 ms at 48 kHz/256-frame, both well under the 100 ms bound. Task 6's latency test measures the shipped rule and asserts these numbers.
