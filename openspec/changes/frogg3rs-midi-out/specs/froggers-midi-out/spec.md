...
K = 1 is the only count at or under 100 ms at both block sizes. The note-on and the note-off before it are stamped at the frame where the detector reported, so the latency on the output timeline is the figure above, with no further delay from the block. The worst note latency SHALL be at or under 100 ms at host blocks of 128 and 256 frames. These figures were measured without the note-off on a quiet output described below. The latency test runs a 20-step render (a factor of 1.5 on the default patch's three VCO pitch knobs, target note 52, one step per quarter note, 5% into the gate-open window) and asserts the worst latency over all 20 steps is at or under 100 ms at both block sizes; at HEAD it prints a worst latency of 93.7292 ms at 128 frames and 94.3958 ms at 256 frames, from that render.
