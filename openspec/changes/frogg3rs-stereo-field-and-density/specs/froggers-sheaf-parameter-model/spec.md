...
Restated because its explanatory paragraph carried a Reverb measurement that does not describe the wet path it was cited for. The figure was taken with the page's Send at its registered default, which is closed, so the tank was never fed and the number measured the page's own output limiter acting on a dry pass-through. The master is inert at that operating point as well: a closed Send drives the wet authority to zero. The two figures could not describe one run in any case -- for the scalar gain that path is, the second is forced by the first, and the stated pair misses by six hundredths of a decibel. The sentence is dropped rather than re-measured into prose, because a figure in prose cannot fail when it drifts. What the paragraph asserts is unchanged and remains true: no page's wet path is transparent at rest.

No `RESTATES-EXCEPT` entry accompanies this edit, and that is not an omission. The restate gate compares BULLETS -- its `join_wrapped` collects only lines opening with a dash, so a requirement's prose paragraphs are never compared and never counted as dropped. Declaring a prose sentence there matches nothing by construction and fails the gate. Every bullet and every scenario under this requirement is carried forward verbatim, which the gate does check.
