...
offers it only as two modulation sources, the raw signal and its envelope follower. Honestly, this is because I got lazy and didn't want to recreate the guitar pedal design from scratch, but your external audio controlling any parameter you want has a similar effect with fewer potential microphone feedback issues, so this isn't really a problem for me.
...
Each modulation depth parameter can go down up to two more layers deep carrying those same fifteen sources. The amount by which one source modulates another can be modulated, and so can that modulation's own amount.
...
Randomize All goes one modulation level deep at a time. To randomize the level below, open a
modulation view and press Randomize All again there. Because each modulation depth is a real parameter that has to be
...
being smooth and values snap between newly crispy-crunchy intervals. This works on human knob-turning as well as parameters patched through modulation sources.

**Crunchy** is never randomized: only you can turn it and set its modulation depths. **Randomize All** randomizes the Crispy of at most two of the six pages per press, never all six at once, because scrambling all six together sounds a lot like randomizing Crunchy. **Randomize Page** can also randomize the Crispy value of the page on screen.
