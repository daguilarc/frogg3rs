...
FroggersPluginProcessor::FroggersPluginProcessor(synth::RuntimeDataPaths dataPathsForTest)
    ...
    // discovery reads dataPaths_ during `Engine::Initialize`
    // (External/Sheaf/projects/synth/include/synth/Engine.hpp's
    ...
    // same as Sheaf Runtime.hpp (`Runtime::Start()`,
    // External/Sheaf/projects/synth/runtime/Runtime.hpp, starts it at
...
std::uint64_t FroggersPluginProcessor::NowMicros() const {
    // Same derivation as Runtime.hpp's own `NowMicros()`
    // (External/Sheaf/projects/synth/runtime/Runtime.hpp): a
...
void FroggersPluginProcessor::prepareToPlay(double sampleRate, int samplesPerBlock) {
    // Runtime.hpp's `audioDeviceAboutToStart`
    // (External/Sheaf/projects/synth/runtime/Runtime.hpp): engine_.Prepare() is
...
void FroggersPluginProcessor::releaseResources() {
    ...
    // `audioDeviceStopped()` override
    // (External/Sheaf/projects/synth/runtime/Runtime.hpp) only clears a diagnostic counter
    // (activeInputChannels_) that this plugin has no equivalent of (its
    // input bus is optional and disabled by default, not absent -- the
    // constructor declares a stereo input bus a host may enable).
    // synth::Engine/FroggersAppCore
...
void FroggersPluginProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages) {
    ...
    // asserts it equals config_.numAudioInputs exactly, inside `Engine::ProcessBlock`
    // (External/Sheaf/projects/synth/include/synth/Engine.hpp), so
    ...
    // during `Engine::ProcessBlock`
    // (External/Sheaf/projects/synth/include/synth/Engine.hpp) -- left default-constructed
    ...
    // message-thread juce::Timer, in its own `timerCallback()`, never the audio
...
void FroggersPluginProcessor::timerCallback() {
    ...
    // while `syncConfig_.receiveClock` is true, inside `MasterClock::SetTempoBpm`
    // (External/Sheaf/projects/synth/src/MasterClock.cpp) -- that suppression is a property of the flag, not of HOW
    // the tempo estimate itself gets updated. `MasterClock::
    // HandleExternalClock` (External/Sheaf/projects/synth/src/MasterClock.cpp) is the only path
    ...
    // `MasterClock::HandleExternalClock`, inside `Engine::RouteRealtimeBatch`
    // (External/Sheaf/projects/synth/include/synth/Engine.hpp), and
    ...
    // ApplySyncConfig call, inside `Engine::ProcessBlock`
    // (External/Sheaf/projects/synth/include/synth/Engine.hpp) -- both pre-date
    ...
    if (hostTempoUsable) {
        ...
        // (External/Sheaf/projects/synth/src/ParameterModulation.cpp), so a burst of correctly-
        ...
        // (`ExpireExternalSource`/`ClearExternalSource`,
        // External/Sheaf/projects/synth/src/MasterClock.cpp) is reactive, not ambient: it only runs from inside
        // `AcceptExternalSource`, itself only reached when ANOTHER clock/
        // transport message arrives (also in External/Sheaf/projects/synth/src/MasterClock.cpp) -- if the
