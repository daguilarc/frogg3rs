...
// hard `static constexpr` (External/Sheaf/projects/synth/include/synth/StandardModulators.hpp) with four
// hardcoded visualizers (`StandardModulators::StandardModulators`'s
// randomVisualizer0_..3_ fields, same file), so it cannot yield six S&H sources,
...
// (External/Sheaf/projects/synth/src/ParameterModulation.cpp, reached here through
// `ParameterGroup::SetModulationSource`, the same public wrapper). Standalone
// Sheaf pieces reused: `NoiseModulatorProcessor` (External/Sheaf/projects/synth/include/synth/DspNoise.hpp),
// `NoiseWaveformVisualizer`, `GangedRandomLfoVisualizer`.
//
...
// throws (`External/Sheaf/projects/synth/include/synth/DspRandomLfo.hpp`).
//
// ============================================================================
...
// `DefaultRandomDrawSource::Uniform01()` (External/Sheaf/projects/synth/include/synth/DspRandomLfo.hpp,
// std::uniform_real_distribution<float>{0.0f,1.0f}) and
// `NoiseModulatorProcessor::Process()` writes `random_.UniformOpen01()`
// (External/Sheaf/projects/synth/include/synth/DspNoise.hpp) -- both already [0,1].
...
// rather than passing raw bipolar signal through. This
...
// (External/Sheaf/projects/synth/include/synth/ParameterModulation.hpp) plus one bool computed from it
// (`ShowingModulation()`, same file); `Bank::HandlePress` opens a
// modulation view for ANY non-selected pressed cell regardless of how deep
// the current view already is (`OpenModulationView`, External/Sheaf/projects/synth/src/ParameterModulation.cpp), so it will
...
//
// ============================================================================
// Randomize: single-cell press dispatch reaches Sheaf's private mutator;
// bulk depth randomization does not
// ============================================================================
// `Bank::RandomizeModulationDepths` and the `Modifier::Random`/`RandomMod`
// dispatch inside `Bank::HandlePress` and `Bank::RandomizeModulationDepths`
// (External/Sheaf/projects/synth/src/ParameterModulation.cpp) are PRIVATE to `Bank` -- there is no public API to invoke them
...
// trigger a bank-wide randomize, same file) is: hold the desired modifier
...
// (also in External/Sheaf/projects/synth/src/ParameterModulation.cpp) applies `Modifier::Random`/`RandomMod` to that ONE cell's
// parameter and returns before touching `selected_`/level state at all. For
// that single-cell dispatch shape, the app "chooses the target set" (which
// cells get pressed, and in what view) while Sheaf performs every mutation.
//
// Randomize All/Page's bulk depth randomization below does not go through
// this mechanism: `RandomizeParameterModulationDepths` draws its own count
// (`DrawGeometricCount`, this app's own reimplementation of the same
// coin-flip shape, redone as a distinct-source draw) and calls the PUBLIC
// `Parameter::EnsureModulationDepth`/`Parameter::RandomizeVisibleValue`
// directly -- no press, no `Bank::HandlePress` dispatch. This app is the
// mutator there; see that function's own comment for why.
//
...
// under a held `Modifier::Random` press, External/Sheaf/projects/synth/src/ParameterModulation.cpp)
...
// own bipolar audio/LFO modulation sources (`NormalizeMatrixOutput`,
// External/Sheaf/projects/synth/apps/braid-4/Braid4Core.hpp), so a
...
// (topLevel_.size(), also in External/Sheaf/projects/synth/src/ParameterModulation.cpp) -- 11 for every
...
// (also in External/Sheaf/projects/synth/src/ParameterModulation.cpp), so the single-arg overload throws
...
public:
    ...
    void Init(synth::ParameterGroup& group, std::size_t extraDepthCapacity = kDepthParameterStorageCapacity) {
        ...
        // This materialization ceiling (kDepthParameterStorageCapacity,
        // 96 x 15, the storage provided at launch, which the message thread
        // adds to as it runs low) exceeds the kMaxParameters=96 initial
    ...
    void PrepareBlockClock(std::optional<double> quarterNotesPerSample) {
        ...
        // on invalid input, External/Sheaf/projects/synth/include/synth/DspRandomLfo.hpp) even with no clock plan
    ...
    void Step(const VcoDrive& vco1, const VcoDrive& vco2, const VcoDrive& vco3,
        ...
        // `multiplier <= 0` (External/Sheaf/projects/synth/include/synth/DspPhasor2Tick.hpp), so rate
    ...
    // behavior, `Bank::OpenModulationView`/`Bank::EnsureModulationDepthParameter`,
    // External/Sheaf/projects/synth/src/ParameterModulation.cpp), so the slate
...
private:
    ...
    void StepClockDrivenLanes(std::optional<double> transportQuarterNotes) {
        ...
        // External/Sheaf/projects/synth/include/synth/DspPhasor2Tick.hpp). The period is the one axis of each
    ...
    void RegisterSources() {
        ...
        }
        {
            ...
            // before registering it (`StandardModulators::Init`,
            // External/Sheaf/projects/synth/include/synth/StandardModulators.hpp).
            ...
            // (`GangedRandomLfoAtomicColor`,
            // External/Sheaf/projects/synth/include/synth/DspRandomLfo.hpp), even though the ModulatorMetadata
...
public:
    ...
    void PressEncoder(synth::PhysicalEncoderId encoderId) {
        ...
        // construction of the Target/Back cell (the selected
...
// (External/Sheaf/projects/synth/src/ParameterModulation.cpp). That function's own loop --
//     while (manager_->NextRandomCoin() < 0.5f) { ... one depth touched ... }
// -- is a geometric distribution over the count of depths touched
...
// (also in External/Sheaf/projects/synth/src/ParameterModulation.cpp) is ABOUT to fire for the next press:
...
// (also in External/Sheaf/projects/synth/src/ParameterModulation.cpp) applies the write to exactly that one
...
// `kNeutralModulationDepthCenter` (External/Sheaf/projects/synth/src/ParameterModulation.cpp) is
...
// (External/Sheaf/projects/synth/src/ParameterModulation.cpp), which is
...
//     (External/Sheaf/projects/synth/src/ParameterModulation.cpp), which sets a bit when the depth
//     is non-null AND `HasNonZeroState()`. `HasNonZeroState()` (same file) returns
...
// `Parameter::RandomizeVisibleValue` (External/Sheaf/projects/synth/src/ParameterModulation.cpp).
...
// `FindVisibleCell(encoderId)->parameter` (External/Sheaf/projects/synth/src/ParameterModulation.cpp),
// the same lookup `Bank::HandlePress`'s modifier branch uses
...
// (External/Sheaf/projects/synth/src/ParameterModulation.cpp), does not work here: that function deltas against
...
// External/Sheaf/projects/synth/src/ParameterModulation.cpp), so seeded/reproducible randomize is
// unaffected.
//
